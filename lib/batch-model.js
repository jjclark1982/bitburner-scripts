 ...
 */
 export class Batch extends Array {
    ...
    scheduleForSafeWindows(tDelta, expectedSecurity) {
        ...
        // a safe window is any time that expectedSecurity <= job.result.prepDifficulty
        function findSafeStartTime(startTime, prepDifficulty) {
            let i;
            for (i = 0; i < expectedSecurity.length; i++) {
                if (expectedSecurity[i][0] > startTime) {
                    break;
                }
            }
            // now i is the first index after startTime
            for (i = i-1; i < expectedSecurity.length; i++) {
                const [t, sec] = expectedSecurity[i];
                if (sec > prepDifficulty) {
                    console.log(`job start security was expected to be ${sec} > ${prepDifficulty} at ${startTime}`);
                    continue;
                }
                if (t > startTime) {
                    console.log(`shifted startTime by ${(t + tDelta/2 - startTime).toFixed(1)} ms to a safe window`);
        ...
        for (const job of this) {
            ...
            job.startTime = findSafeStartTime(job.startTime, job.result.prepDifficulty);
